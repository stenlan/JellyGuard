        ...
        where TGlobalData : struct
    {
        ...
        protected async Task SaveGlobalData(TGlobalData globalData)
        {
            ...
            await using (dbContext)
            {
                ...
                data = authProviderDatas.First(provider => provider.AuthenticationProviderId == typeName);
                data.Data = JsonSerializer.Serialize(globalData, JsonDefaults.Options);
                authProviderDatas.Update(data);
        ...
        protected async Task<TUserData?> GetUserData(User user)
        {
            ...
            var userId = user.Id;
            var dbContext = await contextFactory.CreateDbContextAsync().ConfigureAwait(false);
            UserAuthenticationProviderData? data;

            await using (dbContext)
            {
                data = dbContext.UserAuthenticationProviderDatas().FirstOrDefault(provider => provider.AuthenticationProviderId == typeName && provider.UserId == userId);
            }
